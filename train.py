...
from keras_pre_img import flip_axis, random_channel_shift
...
class Learner(object):
    ...
    test_mask_gt = os.path.join(res_dir, 'test_mask_gt.npy')
    ...
    def split_train_and_valid(cls, data, mask, validation_split, shuffle=False):
        ...
        x_valid = data[split_at:]
        y_train = mask[:split_at]
        y_valid = mask[split_at:]
...
#                y_train.append(_y.reshape((1,) + _y.shape))
    ...
    def train_and_predict(self, pretrained_path=None, split_random=True):
        ...
        (x_train, y_train), (x_valid, y_valid) = self.split_train_and_valid(imgs_train, imgs_mask_train,
                                                        validation_split=self.validation_split)
